...
output_dir = './attenuation_coeffs_sweep' # 'dataset_for_segmentation'  # './variable_dists_and_sigma_s'
...
def main():
    ...
    sigma_s = [(float(i)/200,float(i)/200,float(i)/200) for i in range(100)]
    ...
    with open('{}/log.txt'.format(output_dir), 'wt') as file:
        ...
        for copy, sig_s in product(list(range(copies)), sigma_s):
            ...
            for f in [config_file_path]:
                ...
                print(cmd)
                os.system(cmd)
            file.write('{:03}:distance_1={}:distance_2={}:sigma_s={}:sigma_a={}:angle={}\n'.format(idx, d1, d2, sig_s, sig_a, target_ang))
            file.flush()
            idx += 1
